...
from calib3d.calib3d.points import Point2D, Point3D, HomogeneousCoordinatesPoint
...
class Calib:
    def __init__(
        ...
        **_,
        ...
        self.K = K
        self.kc = np.array(
            (kc if kc is not None else [0, 0, 0, 0, 0]), dtype=np.float64
        )
        self.R = R
    ...
    def from_P(cls, P, width, height) -> "Calib":
        ...
        Returns:
            A Calib object
        """
        K, R, T, Rx, Ry, Rz, angles = cv2.decomposeProjectionMatrix(
            P
    ...
    def _project_3D_to_2D_cv2(self, point3D: Point3D):
        raise BaseException(
            "This function gives errors when rotating the calibration..."
        )
        return Point2D(
            cv2.projectPoints(
                point3D,
    ...
    def project_3D_to_2D(self, point3D: Point3D) -> Point2D:
        ...
        point2D_H[2] = point2D_H[2] * np.sign(
            point2D_H[2]
        )  # correct projection of points being projected behind the camera
        point2D = Point2D(point2D_H)
        # avoid distortion of points too far away
        excluded_points = np.logical_or(
            np.logical_or(point2D.x < -self.width, point2D.x > 2 * self.width),
    ...
    def project_2D_to_3D(
        self, point2D: Point2D, X: float = None, Y: float = None, Z: float = None
    ) -> Point3D:
        ...
        assert isinstance(
            point2D, Point2D
        ), "Wrong argument type '{}'. Expected {}".format(type(point2D), Point2D)
    ...
    def distort(self, point2D: Point2D) -> Point2D:
        ...
        if np.any(self.kc):
            ...
            dx = np.array(
                [
                    2 * tan1 * point2D.x * point2D.y
                    + tan2 * (r2 + 2 * point2D.x * point2D.x),
                    2 * tan2 * point2D.x * point2D.y
                    + tan1 * (r2 + 2 * point2D.y * point2D.y),
                ]
            ).reshape(2, -1)
    ...
    def rotate(self, angle) -> "Calib":
        ...
        if angle == 0:
            return self
        A, new_width, new_height = compute_rotate(
            self.width, self.height, angle, degrees=True
        )
    ...
    def rot90(self, k) -> "Calib":
        ...
        return self.update(
            K=R @ self.K,
            width=self.height if transpose else self.width,
            height=self.width if transpose else self.height,
        )
    ...
    def compute_length2D(self, point3D: Point3D, length3D: float) -> np.ndarray:
        ...
        assert np.isscalar(
            length3D
        ), f"This function expects a scalar `length3D` argument. Received {length3D}"
        point3D_c = Point3D(
            np.hstack((self.R, self.T)) @ point3D.H
        )  # Point3D expressed in camera coordinates system
        orth = np.dot(
            point3D_c.flatten(), Point3D(point3D_c.x, point3D_c.y, 0).flatten()
        )
        orth = orth * length3D / np.linalg.norm(orth)
        point3D_c.y += (
            orth  # add the 3D length orthogonally to the point in camera coordinates
        )
    ...
    def compute_length3D(self, point3D: Point3D, length2D: float) -> np.ndarray:
        ...
        assert np.isscalar(
            length2D
        ), f"This function expects a scalar `length2D` argument. Received {length2D}"
    ...
    def projects_in(self, point3D: Point3D) -> np.ndarray:
        ...
        cond = np.stack(
            (
                point2D.x >= 0,
                point2D.y >= 0,
                point2D.x <= self.width - 1,
                point2D.y <= self.height - 1,
            )
        )
    ...
    def is_behind(self, point3D: Point3D) -> np.ndarray:
        ...
        n = Point3D(
            0, 0, 1
        )  # normal to the camera plane in camera 3D coordinates system
        point3D_c = Point3D(
            np.hstack((self.R, self.T)) @ point3D.H
        )  # point3D expressed in camera 3D coordinates system
...
def rotate_image(image, angle):
    ...
    return cv2.warpAffine(
        image, A[0:2, :], (new_width, new_height), flags=cv2.INTER_LINEAR
    )


def parameters_to_affine_transform(
    angle: float,
    x_slice: slice,
    y_slice: slice,
    output_shape: tuple,
...
def compute_shear_rectification_matrix(calib: Calib, point2D: Point2D):
    ...
    R1 = np.eye(3)
    R1[0:2, :] = cv2.getRotationMatrix2D(
        point2D.to_int_tuple(), np.arctan2(vector.y, vector.x) * 180 / np.pi, 1.0
    )
    ...
    R2 = np.eye(3)
    R2[0:2, :] = cv2.getRotationMatrix2D(
        (0, 0), -np.arctan2(vector.y, vector.x) * 180 / np.pi, 1.0
    )
